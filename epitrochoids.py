...
class Epitrochoids:
    ...
    def _process_parameters(self, d, n_values_in, r_values_in):
        ...
        if r_values_in is not None:
            ...
            for r in input_r_list:
                ...
                r_was_irrational_flags.append(
                    self._is_irrational(r, denom_threshold=1000)
                )
            ...
            if not processed_n_values:
                raise ValueError(
                    "No valid rolling circles could be determined from input r_values."
                )
            ...
            if n_values_in is None:
                ...
                if self.type == "rose":
                    ...
                    if d is not None:
                        warnings.warn(
                            "Type is 'circle', ignoring provided 'd'. Using d=0."
                        )
            ...
            for n_raw in input_n_list:
                ...
                if n_numeric == 0:
                    warnings.warn(
                        "n=0 detected, results in infinite radius 'b'. Skipping."
                    )
                    continue
                ...
                if self.type == "limacon" and abs(n_numeric - 1.0) > 1e-6:
                    warnings.warn(
                        f"Type is 'limacon', requires n=R/r=1. Adjusting input n={n_numeric} to 1.0."
                    )
                    n_numeric = 1.0
            ...
            if not valid_n_numeric:
                raise ValueError(
                    "No valid rolling circles could be determined from input n_values."
                )

            processed_r_values = temp_r_values
            processed_n_values = valid_n_numeric  # Store numeric values
            self.n_values_input_type = (
                valid_n_input_type  # Store input types separately
            )

        num_circles = len(processed_r_values)
        self.b_values = processed_r_values
        self.n_values_numeric = (
            processed_n_values  # Store numeric n for calcs/type checking
        )
        ...
        for i, n_numeric in enumerate(self.n_values_numeric):  # Iterate using numeric n
            ...
            n_input_val = (
                self.n_values_input_type[i]
                if i < len(self.n_values_input_type)
                else n_numeric
            )  # Get original input if available

            # Check if r was flagged as irrational (only if r_values were the input source)
            if (
                r_values_in is not None
                and i < len(r_was_irrational_flags)
                and r_was_irrational_flags[i]
            ):
                print(
                    f"Input r={self.b_values[i]:.4f} flagged as irrational/complex. "
            ...
            # Also check if n was input directly as a float likely representing irrational
                ...
                r_values_in is None
                and isinstance(n_input_val, float)
                and self._is_irrational(n_input_val)
            ):
                print(
                    f"Input n={n_input_val:.4f} flagged as irrational/complex float. Forcing treatment as irrational."
                )
                determined_n_type = "irrational"
                determined_revolutions = self.max_revolutions_irrational
                is_forced_irrational = True
                ...
                try:
                    ...
                    if np.isclose(n_numeric, round(n_numeric)):
                        ...
                        if isinstance(n_input_val, Fraction):
                            ...
                            if (
                                n_input_val.denominator <= 1000
                            ):  # Threshold for 'simple' rational
                        ...
                        if not is_rational_simple and not self._is_irrational(
                            n_numeric, denom_threshold=1000
                        ):
                            # It's representable by a fraction with a reasonably small denominator
                            frac_n = Fraction(n_numeric).limit_denominator(
                                1000
                            )  # Find that fraction
            ...
            if self.type == "circle":
                ...
                print(
                    f"Type is 'circle', using n={n_numeric:.4f} (classified as {determined_n_type}) "
        ...
        if self.type == "epicycloid":
            ...
            if d is not None:
                warnings.warn("Type is 'epicycloid', ignoring provided 'd'. Using d=r.")
        elif self.type == "rose":
            final_d_values = [self.a + b for b in self.b_values]
            if d is not None:
                warnings.warn(
                    "Type is 'rose', ignoring provided 'd'. Using d=R+r (a+b)."
                )
        elif self.type == "limacon":
            ...
            if d is None:
                ...
                warnings.warn(
                    "Type is 'limacon' but 'd' not provided. Using default d=R (h=a) -> Cardioid."
                )
                ...
                if len(d) != num_circles:
                    ...
                    if n_values_in is None and r_values_in is None:
                        ...
                        raise ValueError(
                            f"Length of 'd' ({len(d)}) must match the number of rolling circles ({num_circles}) "
            ...
            if d is None:
                ...
                warnings.warn(
                    "Parameter 'd' not provided for random type. Using default d=r/2."
                )
            elif isinstance(d, (int, float)):
                final_d_values = [float(d)] * num_circles
            else:  # d is a list
                ...
                if len(d_float) != num_circles:
                    ...
                    if len(d_float) < num_circles:
                        warnings.warn(
                            f"Length of 'd' ({len(d_float)}) < number of circles ({num_circles}). "
                            "Repeating last 'd' value."
                        )
                        final_d_values = d_float + [d_float[-1]] * (
                            num_circles - len(d_float)
                        )
        ...
        if self.save_anim and self.filename is None:
            ...
            n_info = "_".join(
                [f"n{n:.3f}".replace(".", "p") for n in self.n_values_numeric]
            )
    ...
    def _calculate_animation_parameters(self):
        ...
        print(
            f"Animation requires max {self.max_total_revolutions:.2f} revolutions of the center."
        )

        self.theta_max_pattern = 2 * np.pi * self.max_total_revolutions
        # Ensure total_frames is integer
        self.total_frames = int(
            np.ceil(self.max_total_revolutions * self.frames_per_revolution)
        )
        if self.total_frames <= 0:
            self.total_frames = self.frames_per_revolution
    ...
    def _setup_plot(self):
        ...
        # Stationary circle
        self.fixed_circle = patches.Circle(
            (0, 0), self.a, fill=False, color="gray", lw=1.5, ls="--"
        )
        ...
        if len(self.b_values) == 1:
            ...
            len(self.b_values) <= 10
        ):  # Use perceptually uniform viridis for few colors
        ...
        for i, (b, h, n_num, n_typ) in enumerate(
            zip(self.b_values, self.h_values, self.n_values_numeric, self.n_types)
        ):
            color = colors[i]

            # Rolling circle patch
            circle = patches.Circle(
                (self.a + b, 0), b, fill=False, color=color, lw=1.5, zorder=3
            )
            ...
            if n_typ == "rational":
                ...
                frac_n = (
                    ...
                    else Fraction(n_num).limit_denominator(1000)
                )
            ...
            (radius_line,) = self.ax.plot(
                [], [], "--", color=color, lw=1.5, alpha=0.8, zorder=4
            )
            ...
            (center_point,) = self.ax.plot(
                [], [], "o", color="white", ms=5, alpha=0.6, zorder=4
            )
            self.center_points.append(center_point)

        if len(self.b_values) > 0:  # Add legend only if there are curves
            self.ax.legend(
                loc="upper right", fontsize=9, facecolor="#1C1C1C", framealpha=0.7
            )

        # Add text annotation for revolution counter if animation > 1 rev
        if (
            self.max_total_revolutions > 1.01
        ):  # Add threshold to avoid showing for exactly 1 rev
            self.rev_text = self.ax.text(
                0.02,
                0.98,
                "",
    ...
    def _init_animation(self):
        ...
        for i in range(len(self.b_values)):
            ...
            init_positions = self._epitrochoid_points(
                0, self.a, self.b_values[i], self.h_values[i]
            )
            ...
            self.radius_lines[i].set_data(
                [init_center_x, init_trace_x], [init_center_y, init_trace_y]
            )

            all_elements.extend(
                [
                    self.path_lines[i],
                    self.circles_rolling[i],
                    self.radius_lines[i],
                    self.tracing_points[i],
        ...
        if self.rev_text is not None:
            ...
            total_rev_display = (
                f"{int(self.max_total_revolutions)}"
                if np.isclose(
                    self.max_total_revolutions, round(self.max_total_revolutions)
                )
    ...
    def _animate_frame(self, frame):
        ...
        for i, (b, h) in enumerate(zip(self.b_values, self.h_values)):
            ...
            (centers_x_hist, centers_y_hist, traces_x_hist, traces_y_hist) = (
                self._epitrochoid_points(theta_history, self.a, b, h)
            )
            ...
            if len(traces_x_hist) > 0:
                ...
                (
                    ...
                    current_trace_x,
                    current_trace_y,
                ) = self._epitrochoid_points(0, self.a, b, h)
            ...
            self.radius_lines[i].set_data(
                [current_center_x, current_trace_x], [current_center_y, current_trace_y]
            )
            ...
            try:
                with tqdm(
                    ...
                    def progress_update(current_frame, total_frames):
                        pbar.n = (
                            current_frame + 1
                        )  # Update tqdm progress (callback is 0-based)
                        pbar.refresh()  # Force redraw

                    self.anim.save(
                        str(filepath),
                        writer=writer_choice,
                        fps=self.FPS,
                        dpi=100,
                ...
                print(
                    "If using blit=True, try setting blit=False if errors occur during save."
                )
...
if __name__ == "__main__":
    ...
    random_epi_anim = Epitrochoids(R=4, d=1, n_values=[3, 2.25, 1.75], save_anim=False)
    ...
    custom_r_epi_anim = Epitrochoids(
        R=4,
        r_values=[1.0, 5 / 3, math.pi],
        d=[0.5, 1.5, 2.5],
        type="random",
        save_anim=True,
    ...
    epi_irrational_anim = Epitrochoids(
        R=3,
        n_values=math.e,
        type="epicycloid",
        save_anim=True,
    ...
    epicycloid_irrational_anim = Epitrochoids(
        R=3,
        n_values=[math.sqrt(2), math.pi, math.e],
        type="epicycloid",
        save_anim=True,
        max_revolutions_irrational=10,
        frames_per_rev=100,
    ...
    mixed_epi_anim = Epitrochoids(
        R=3,
        n_values=[3, 5 / 2, math.pi],
        type="epicycloid",
        save_anim=True,
    ...
    limacon_anim = Epitrochoids(
        R=2, d=[0.5, 1, 2, 3, 4], type="limacon", save_anim=True, frames_per_rev=200
    )
    # limacon_anim.generate_animation()

    # --- Rose Example (h = a + b) ---
    print("\nGenerating Rose from Epitrochoids (h=a+b)...")
    rose_anim = Epitrochoids(
        R=3, n_values=10, type="rose", save_anim=True, frames_per_rev=500, fps=50
    )
    # rose_anim.generate_animation()

    # Rose with rational n values
    print("\nGenerating Rose with rational n values...")
    rose_rational_anim = Epitrochoids(
        R=3, n_values=[3 / 2, 7 / 4, 11 / 5], type="rose", save_anim=True, fps=50
    )
    # rose_rational_anim.generate_animation()

    # --- Circle Example (h = 0)
    print(
        "\nGenerating Circle from Epitrochoid(h = 0)"
    )  # h=0, the tracing point lies at the center of the rolling circle
    circle_anim = Epitrochoids(
        R=4, n_values=[2, 4, math.e], type="circle", save_anim=True
    )
