...
class Hypotrochoids:
    ...
    def _process_parameters(self, d, n_values_in, r_values_in):
        ...
        if r_values_in is not None:
            ...
            for r in input_r_list:
                ...
                if r > self.a and self.type != "ellipse":  # Allow r=a/2 for ellipse
                    warnings.warn(
                        f"Rolling radius r={r:.3f} > R={self.a:.3f} (n<1). "
                        "Rolling circle is larger than fixed circle. "
                        "Ensure this is intended for internal rolling."
                    )
                if self.type == "ellipse" and abs(self.a / r - 2.0) > 1e-6:
                    warnings.warn(
                        f"Type is 'ellipse', requires R/r = 2. Adjusting r={r:.3f} to {self.a / 2.0:.3f}."
                    )
            ...
            n_values_input_type = (
                processed_n_values  # Use numeric n as stand-in for 'input type'
            )
            ...
            for n_raw in input_n_list:
                ...
                if n_numeric < 1 and self.type != "ellipse":  # n=2 for ellipse is >= 1
                    warnings.warn(
                        f"n={n_numeric:.3f} < 1 detected (r > R). "
                        "Rolling circle is larger than fixed circle. "
                        "Ensure this is intended for internal rolling."
                    )
                ...
                if self.type == "ellipse" and abs(n_numeric - 2.0) > 1e-6:
                    warnings.warn(
                        f"Type is 'ellipse', requires n=R/r=2. Adjusting input n={n_numeric:.3f} to 2.0."
                    )
                    n_numeric = 2.0
        ...
        for i, n_numeric in enumerate(self.n_values_numeric):
            ...
            n_input_val = (
                self.n_values_input_type[i] if r_values_in is None else n_numeric
            )
            ...
            # Check if n was input directly as a float likely representing irrational
                ...
                and self._is_irrational(n_input_val, denom_threshold=1000)
            ):
                print(
                    f"Input n={n_input_val:.4f} flagged as irrational/complex float. "
                    "Forcing treatment as irrational."
                )
                n_type = "irrational"
                revolutions = self.max_revolutions_irrational
                is_forced_irrational = True
                ...
                try:
                    ...
                    # ONLY check for rational IF it wasn't classified as integer
                        ...
                        if r_values_in is None and isinstance(n_input_val, Fraction):
                            ...
                            if (
                                n_input_val.denominator > 1
                                and n_input_val.denominator <= 1000
                            ):
                                n_type = "rational"
            ...
            n_display = (
                n_input_val if r_values_in is None else n_numeric
            )  # Use input n if available else numeric

            if n_type == "irrational":
                # Display original input if it was float, otherwise show numeric
                input_display_str = (
                    f"{float(n_display):.4f}"
                    if isinstance(n_display, (float, np.floating))
                    else str(n_display)
                )
                print(
                    f"n={n_numeric:.4f} "
                ...
                input_display_str = (
                    str(n_input_val)
                    if isinstance(n_input_val, (int, Fraction))
                    else f"{float(n_display):.4f}"
                )
                print(
                    f"n={int(n_numeric)} "
        ...
        if self.save_anim and self.filename is None:
            ...
            r_info = (
                "_".join([f"r{r:.3f}".replace(".", "p") for r in self.b_values])
                if r_values_in is not None
                else ""
            )
    ...
    def _calculate_animation_parameters(self):
        ...
        max_reach = max(
            [abs(self.a - b) + h for b, h in zip(self.b_values, self.h_values)],
            default=self.a,
        )
        ...
        if self.total_frames <= 0:
            self.total_frames = self.frames_per_revolution  # Ensure minimum frames
    ...
    def _hypotrochoid_points(theta, a, b, h):
        ...
        if b == 0:  # Avoid division by zero
            ...
            if isinstance(theta, (np.ndarray)):  # Match shape for array input
                return (
                    np.full_like(theta, center_x),
                    np.full_like(theta, center_y),
                    np.full_like(theta, trace_x),
                    np.full_like(theta, trace_y),
                )
            else:  # Scalar input
        ...
        trace_angle_term = (
            (a - b) / b * theta
        )  # Angle for the tracing point calculation
        ...
        trace_y = center_y - h * np.sin(
            trace_angle_term
        )  # Minus sign for internal rolling
    ...
    def _setup_plot(self):
        ...
        for i, (b, h, n_val, n_typ) in enumerate(
            ...
            circle = patches.Circle(
                (self.a - b, 0), b, fill=False, color=color, lw=1.5, alpha=0.8, zorder=3
            )
            ...
            if n_typ == "rational":
                ...
                frac_n = (
                    ...
                    else Fraction(n_val).limit_denominator(1000)
                )
    ...
    def _init_animation(self):
        ...
        for i in range(len(self.b_values)):
            ...
            init_position = self._hypotrochoid_points(
                theta=0, a=self.a, b=self.b_values[i], h=self.h_values[i]
            )
    ...
    def _animate_frame(self, frame):
        ...
        for i, (b, h) in enumerate(zip(self.b_values, self.h_values)):
            ...
            (centers_x_hist, centers_y_hist, traces_x_hist, traces_y_hist) = (
                self._hypotrochoid_points(theta_history, self.a, b, h)
            )
            ...
            if len(traces_x_hist) > 0:
                ...
                (
                    ...
                    current_trace_x,
                    current_trace_y,
                ) = self._hypotrochoid_points(0, self.a, b, h)
...
if __name__ == "__main__":
    ...
    custom_random = Hypotrochoids(
        R=5,
        r_values=[1.5, 1, math.pi],  # n = 5/1.5=10/3, n=5/1=5, n=5/pi=1.5915..
        d=[0.5, 1, 1.5],
        type="random",
        save_anim=False,
    ...
    hypo_anim = Hypotrochoids(
        R=4, n_values=[3, 4, 5], type="hypocycloid", save_anim=False
    )
    # hypo_anim.generate_animation()

    # --- Rational n ---
    print("\nGenerating Hypocycloid (Rational n=3.5 = 7/2)...")
    hypo_rational = Hypotrochoids(
        R=4, n_values=Fraction(7, 2), type="hypocycloid", save_anim=False
    )
    ...
    hypo_rational_mixed = Hypotrochoids(
        R=4,
        n_values=[5 / 2, Fraction(11, 4), 18 / 5],
        type="hypocycloid",
        save_anim=False,
        fps=50,
    )
    ...
    mixed_anim = Hypotrochoids(
        R=5,
        n_values=[3, Fraction(7, 2), math.pi],
        type="hypocycloid",
        max_revolutions_irrational=5,
        save_anim=True,
        fps=40,
    ...
    ellipse_anim = Hypotrochoids(
        R=6,
        d=[0.5, 1.25, np.sqrt(3)],
        type="ellipse",
        save_anim=True,
    ...
    ellipse_anim_single = Hypotrochoids(
        R=4, d=1, n_values=2, type="ellipse", save_anim=False
    )
    ...
    rose_anim_rational = Hypotrochoids(
        R=4,
        n_values=[Fraction(4, 3), Fraction(5, 4), Fraction(7, 5)],
        type="rose",
        frames_per_rev=100,
        fps=30,
        save_anim=False,
